            ...
            <View className={styles.item} key={item.id}>
              ...
              <View>
                <AtButton
                  size="small"
                  type="primary"
                  onClick={() => Taro.atMessage({ message: "开发中,敬请期待" })}
                >
                  预定
                </AtButton>
              </View>
...
const Trainer: Taro.FC<{ tarlarList: PersonTralarDateType[] }> = ({
  tarlarList,
}) => {
  return (
    <View className={styles.listContainer}>
...
const ShopPage: Taro.FC = () => {
  const [currentType, setCurrentType] = useState<"lession" | "sijiao">(
    "lession"
  );
  ...
  useEffect(() => {
    ...
    if (params?.type === "sijiao") {
      initTralarData({ pageNo: 1, pageSize: 100, venueId: venueDetail.id });
    } else {
      initLessionData({
        pageNo: 1,
        venueId: venueDetail.id,
        date: tabList[0].fullDate,
        pageSize: 100,
      });
    }
  }, []);
